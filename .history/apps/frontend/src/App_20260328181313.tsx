...
import { Users, CalendarDays, Briefcase, Globe, LogOut, ChevronLeft, ChevronRight, ChevronDown, Settings, ShieldCheck, ClipboardList, CalendarClock } from 'lucide-react';
import logoFull from './assets/logo-ktpm.svg';
...
function App() {
  ...
  useEffect(() => {
    ...
    async function validateSession() {
      try {
        await getCurrentUser(currentActor);
      } catch {
        if (!cancelled) {
          logout();
        }
  ...
  const availableNavItems = useMemo(() => {
    ...
    if (canAccessTasks(actor.role)) {
      items.push({ path: '/tasks', label: t('nav.tasks'), icon: <ClipboardList size={20} className="nav-icon" /> });
    }
    ...
    if (canAccessSchedule(actor.role)) {
      items.push({ path: '/schedule', label: t('nav.schedule'), icon: <CalendarClock size={20} className="nav-icon" /> });
    }
  ...
  const avatarSrc = actor.avatarUrl
    ? actor.avatarUrl
    : `https://ui-avatars.com/api/?name=${encodeURIComponent(actor.fullName)}&background=f3f4f6&color=ff7b25&rounded=true&size=128`;
  ...
  const subModuleItems = useMemo(() => {
    const map: Record<string, Array<{ label: string; path: string }>> = {
      ...
      tasks: [
        { label: t('subMenu.tasks.board'), path: '/tasks?view=board' },
        { label: t('subMenu.tasks.mine'), path: '/tasks?view=mine' },
        { label: t('subMenu.tasks.created'), path: '/tasks?view=created' },
        { label: t('subMenu.tasks.history'), path: '/tasks?view=history' },
      ],
      leave: [
      ...
      schedule: [
        { label: t('subMenu.schedule.today'), path: '/schedule?view=today' },
        { label: t('subMenu.schedule.created'), path: '/schedule?view=created' },
      ],
      recruitment: [
        ...
        { label: t('subMenu.recruitment.interviewHistory'), path: '/recruitment?view=interview-history' },
      ],
      administration: [],
      settings: [],
    };
    ...
    return [
      {
        ...
        items: [
          ...
          { label: t('subMenu.administration.departments'), path: '/administration?view=departments' },
          ...
          { label: t('subMenu.administration.scheduleTypes'), path: '/administration?view=schedule-types' },
        ],
      },
      {
        key: 'access',
      ...
      <aside className="sidebar">
        <div className="sidebar-top">
          <div className="brand-wrap">
            {isSidebarExpanded ? (
              <img src={logoFull} alt="KTPM HRM" className="brand-logo-full" />
            ) : (
              <img src={logoMark} alt="KTPM" className="brand-logo-mark" />
            )}
          </div>
