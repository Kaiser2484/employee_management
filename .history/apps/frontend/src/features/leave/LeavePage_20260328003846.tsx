...
export function LeavePage() {
  ...
  const handleCreateSubmit = async (e: FormEvent) => {
    ...
    try {
      await createLeaveRequest(actor, { leaveType, fromDate, toDate, reason, attachment });
      await loadRequests();
        ...
        {isCreateModalOpen && (
          <div className="employee-modal-backdrop" onClick={() => setIsCreateModalOpen(false)}>
            <section className="employee-modal" onClick={e => e.stopPropagation()}>
              ...
              <form className="employee-create-form" onSubmit={handleCreateSubmit} style={{ padding: '0.5rem 1rem 1rem' }}>
                <div className="form-group row-field">
                  <label style={{ fontWeight: 600, fontSize: '0.9rem', marginBottom: '0.3rem' }}>Loại nghỉ phép</label>
                  <select className="input-control" value={leaveType} onChange={e => setLeaveType(e.target.value)} style={{ width: '100%', marginBottom: '1rem' }}>
                    {LEAVE_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                  </select>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                  <div className="form-group row-field" style={{ marginBottom: 0 }}>
                    <label style={{ fontWeight: 600, fontSize: '0.9rem', marginBottom: '0.3rem' }}>Từ ngày</label>
                    <input type="date" className="input-control" value={fromDate} onChange={e => setFromDate(e.target.value)} required />
                  </div>
                  <div className="form-group row-field" style={{ marginBottom: 0 }}>
                    <label style={{ fontWeight: 600, fontSize: '0.9rem', marginBottom: '0.3rem' }}>Đến ngày</label>
                    <input type="date" className="input-control" value={toDate} onChange={e => setToDate(e.target.value)} required />
                  </div>
                </div>
                <div className="form-group row-field" style={{ flexDirection: 'column', alignItems: 'flex-start', marginBottom: '1rem' }}>
                  <label style={{ fontWeight: 600, fontSize: '0.9rem', marginBottom: '0.3rem' }}>File bằng chứng (Ảnh/PDF)</label>
                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', width: '100%' }}>
                    <input type="file" className="input-control" onChange={handleFileChange} accept="image/*,.pdf" style={{ width: '100%' }} />
                    {attachment && <span style={{ color: 'var(--success)', whiteSpace: 'nowrap', fontSize: '0.9rem' }}>✓ Đã tải file</span>}
                  </div>
                </div>
                <div className="form-group row-field" style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                  <label style={{ fontWeight: 600, fontSize: '0.9rem', marginBottom: '0.3rem' }}>Lý do (Không bắt buộc)</label>
                ...
                <div className="form-actions" style={{ marginTop: '2rem', display: 'flex', justifyContent: 'flex-end', gap: '1rem', borderTop: '1px solid var(--line)', paddingTop: '1.2rem' }}>
          ...
          <div className="employee-modal-backdrop" onClick={() => setSelectedRequest(null)}>
            <div className="delete-confirm-modal members-modal personal-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="members-list">
                ...
                <div className="member-item">
                  <span className="member-name">Minh chứng</span>
                  <span className="member-role">
                    {selectedRequest.attachment ? (
                      <a href={selectedRequest.attachment} target="_blank" rel="noreferrer" style={{ color: 'var(--accent)', textDecoration: 'underline' }}>
                        Xem đính kèm
                      </a>
                    ) : '-'}
                  </span>
