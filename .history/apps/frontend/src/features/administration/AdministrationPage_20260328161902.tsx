...
function buildDefaultScheduleTypes() {
...
function loadScheduleTypes() {
  try {
    ...
    if (!raw) {
      return buildDefaultScheduleTypes();
    }

    const parsed = JSON.parse(raw) as string[];
    return Array.isArray(parsed) && parsed.length ? parsed : buildDefaultScheduleTypes();
  } catch {
    return buildDefaultScheduleTypes();
  }
}
...
export function AdministrationPage() {
  ...
  const [scheduleTypeDraft, setScheduleTypeDraft] = useState('');
  ...
  const pageMeta = useMemo(() => {
    const map: Record<AdminView, { title: string; description: string; icon: React.ReactNode }> = {
      ...
      'schedule-types': {
        title: t('administration.views.scheduleTypes.title'),
        description: t('administration.views.scheduleTypes.description'),
  ...
  useEffect(() => {
    localStorage.setItem(SCHEDULE_TYPE_STORAGE_KEY, JSON.stringify(scheduleTypes));
  }, [scheduleTypes]);
  ...
  function addScheduleType() {
    const normalized = scheduleTypeDraft.trim();
    if (!normalized) {
      return;
    }

    const exists = scheduleTypes.some((item) => item.toLowerCase() === normalized.toLowerCase());
    if (exists) {
      showToast(t('administration.catalog.duplicate'), 'error');
      return;
    }

    setScheduleTypes((current) => [normalized, ...current]);
    setScheduleTypeDraft('');
    showToast(t('administration.catalog.scheduleTypeAdded'), 'success');
  }

  function removeScheduleType(target: string) {
    if (scheduleTypes.length <= 1) {
      showToast(t('administration.catalog.duplicate'), 'error');
      return;
    }

    setScheduleTypes((current) => current.filter((item) => item !== target));
    showToast(t('administration.catalog.scheduleTypeRemoved'), 'success');
  }
  ...
  function renderScheduleTypeManager() {
    return (
      <>
        <div className="admin-catalog-form">
          <input
            ...
            onChange={(event) => setScheduleTypeDraft(event.target.value)}
            placeholder={t('administration.catalog.scheduleTypePlaceholder')}
          />
          <button
            type="button"
            className="mini-btn"
                ...
                <tr key={item}>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <button
                      type="button"
                      className="admin-delete-btn"
                      onClick={() => removeScheduleType(item)}
                    >
                      <Trash2 size={14} />
                      {t('administration.catalog.removeButton')}
                    </button>
  ...
  return (
    <section className="module-grid" style={{ animation: 'fadeUp 0.4s ease', height: '100%' }}>
      <article className="card main-card" style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        {currentView === 'schedule-types' && renderScheduleTypeManager()}
