...
type HistoryQuickRange = 'none' | 'today' | 'last7' | 'last30';
...
export function TasksPage() {
  ...
  const [historyQuickRange, setHistoryQuickRange] = useState<HistoryQuickRange>('none');
  ...
  const filteredTasks = useMemo(() => {
    ...
    const fromTimestamp = historyFromDate
      ? new Date(`${historyFromDate}T00:00:00`).getTime()
      : null;
    const toTimestamp = historyToDate
    ...
    return tasks.filter((task) => {
      ...
      let matchHistoryRange = true;
      if (currentView === 'history' && (fromTimestamp !== null || toTimestamp !== null)) {
        const historyTimeRaw = task.completionConfirmedAt ?? task.endedAt ?? task.updatedAt;
        const historyTimestamp = new Date(historyTimeRaw).getTime();
        const matchFrom = fromTimestamp === null || historyTimestamp >= fromTimestamp;
        const matchTo = toTimestamp === null || historyTimestamp <= toTimestamp;
        matchHistoryRange = Number.isFinite(historyTimestamp) && matchFrom && matchTo;
      }

      return matchView && matchStatus && matchPriority && matchSearch && matchHistoryRange;
    });
  }, [actor?.id, currentView, historyFromDate, historyToDate, priorityFilter, search, statusFilter, tasks]);
  ...
  function applyHistoryQuickRange(range: Exclude<HistoryQuickRange, 'none'>) {
    ...
    const normalizedFrom = range === 'today' ? toDate : formatDateForInput(fromDate);
    setHistoryFromDate(normalizedFrom);
  ...
  const renderList = () => {
    ...
    const rows = isHistoryView
      ? [...filteredTasks].sort((a, b) => {
          const aTimestamp = new Date(a.completionConfirmedAt ?? a.endedAt ?? a.updatedAt).getTime();
          const bTimestamp = new Date(b.completionConfirmedAt ?? b.endedAt ?? b.updatedAt).getTime();
    ...
    <section className="module-grid">
      <article className="card main-card">
        ...
        <div className="task-toolbar">
          <div className="task-filters task-main-filters">
            <input
              className="input-control task-main-search"
              placeholder={t('tasks.searchPlaceholder')}
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
            <select className="input-control task-main-select" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as TaskStatus | 'all')}>
              <option value="all">{t('tasks.filters.allStatus')}</option>
              {STATUS_OPTIONS.map((opt) => (
                <option key={opt.value} value={opt.value}>{statusLabel(opt.value)}</option>
              ))}
            </select>
            <select className="input-control task-main-select" value={priorityFilter} onChange={(e) => setPriorityFilter(e.target.value as TaskPriority | 'all')}>
        ...
        <div className="task-toolbar task-history-toolbar">
          <div className="task-filters task-history-filters">
            <div className="task-history-quick-filters" role="group" aria-label={t('tasks.filters.quickRange', 'Lọc nhanh')}>
              <button
                type="button"
                className={`pill ${historyQuickRange === 'today' ? 'active' : ''}`}
                onClick={() => applyHistoryQuickRange('today')}
              >
                {t('tasks.filters.today', 'Hôm nay')}
              </button>
              <button
                type="button"
                className={`pill ${historyQuickRange === 'last7' ? 'active' : ''}`}
                onClick={() => applyHistoryQuickRange('last7')}
              >
                {t('tasks.filters.last7Days', '7 ngày')}
              </button>
              <button
                type="button"
                className={`pill ${historyQuickRange === 'last30' ? 'active' : ''}`}
                onClick={() => applyHistoryQuickRange('last30')}
              >
                {t('tasks.filters.last30Days', '30 ngày')}
              </button>
            </div>
            <input
              className="input-control task-history-search"
              placeholder={t('tasks.searchPlaceholder')}
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
            <input
              type="date"
              className="input-control task-history-date"
              value={historyFromDate}
              onChange={(e) => {
                setHistoryFromDate(e.target.value);
                setHistoryQuickRange('none');
              }}
              aria-label={t('tasks.filters.fromDate', 'Từ ngày')}
            />
            <input
              type="date"
              className="input-control task-history-date"
              ...
              aria-label={t('tasks.filters.toDate', 'Đến ngày')}
            />
            <button
              type="button"
              className="ghost-btn task-history-clear"
              onClick={() => {
                setHistoryFromDate('');
                setHistoryToDate('');
                setHistoryQuickRange('none');
            ...
            >
              {t('tasks.filters.clearHistoryFilter', 'Xóa lọc ngày')}
            </button>
          </div>
        </div>
        )}
