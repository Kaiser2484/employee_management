...
const SCHEDULE_TYPE_STORAGE_KEY = 'hrm_schedule_types_v1';
...
function loadScheduleTypes() {
  try {
    const raw = localStorage.getItem(SCHEDULE_TYPE_STORAGE_KEY);
    if (!raw) {
      return ['meeting', 'business_trip'];
    }
    const parsed = JSON.parse(raw) as string[];
    return Array.isArray(parsed) && parsed.length ? parsed : ['meeting', 'business_trip'];
  } catch {
    return ['meeting', 'business_trip'];
  }
}
...
function formatScheduleDateRange(startAt: string, endAt: string) {
...
export function SchedulePage() {
  ...
  const [scheduleTypes, setScheduleTypes] = useState<string[]>(loadScheduleTypes);
  ...
  useEffect(() => {
    function handleStorage() {
      setScheduleTypes(loadScheduleTypes());
    }
  ...
  function openCreateModal() {
    const latestTypes = loadScheduleTypes();
    setScheduleTypes(latestTypes);
    setForm((current) => ({
      ...DEFAULT_FORM,
      date: current.date || new Date().toISOString().slice(0, 10),
      endDate: current.endDate || current.date || new Date().toISOString().slice(0, 10),
      type: latestTypes[0] ?? 'meeting',
  ...
  function openScheduleDetail(schedule: ScheduleEntry) {
  ...
  function resolveVisibleUserNames(entry: ScheduleEntry) {
    if (entry.scope !== 'selected_users') {
      return [];
    }

    const userMap = new Map(employees.map((employee) => [employee.id, employee.fullName]));
    return entry.visibleUserIds.map((id) => userMap.get(id) || id);
  }
      ...
      <article className="card main-card">
        ...
        <section className="schedule-agenda-grid">
          {rangeSchedules.map((item) => (
            <article key={item.id} className="schedule-agenda-card" onClick={() => openScheduleDetail(item)}>
              <header>
                <span className="status-pill status-pending">{typeLabel(item.type)}</span>
                <small>{formatScheduleDateRange(item.startAt, item.endAt)}</small>
              </header>
              <h4>{item.title}</h4>
              <p>
              ...
              <small>{t('schedule.clickHint')}</small>
            </article>
          ))}
        ...
        {selectedSchedule && (
          <div className="employee-modal-backdrop" onClick={closeScheduleDetail}>
            <section className="employee-modal schedule-detail-modal" onClick={(event) => event.stopPropagation()}>
              <div className="employee-modal-head">
                <h3>{t('schedule.detail.title')}</h3>
                <button type="button" className="ghost-btn" onClick={closeScheduleDetail}>{t('common.close')}</button>
              </div>
              <div className="schedule-detail-grid">
                <div className="schedule-detail-card">
                  <span>{t('schedule.columns.title')}</span>
                  <strong>{selectedSchedule.title}</strong>
                </div>
                <div className="schedule-detail-card">
                  <span>{t('schedule.columns.type')}</span>
                  <strong>{typeLabel(selectedSchedule.type)}</strong>
                </div>
                <div className="schedule-detail-card">
                  <span>{t('schedule.columns.date')}</span>
                  <strong>{formatScheduleDateRange(selectedSchedule.startAt, selectedSchedule.endAt)}</strong>
                </div>
                <div className="schedule-detail-card">
                  <span>{t('schedule.columns.time')}</span>
                  <strong>
                    {new Date(selectedSchedule.startAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    {' - '}
                    {new Date(selectedSchedule.endAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </strong>
                </div>
                <div className="schedule-detail-card">
                  <span>{t('schedule.columns.location')}</span>
                  <strong>{selectedSchedule.location || '-'}</strong>
                </div>
                <div className="schedule-detail-card">
                  <span>{t('schedule.columns.scope')}</span>
                  <strong>{scopeLabel(selectedSchedule.scope)}</strong>
                </div>
                <div className="schedule-detail-card">
                  <span>{t('schedule.columns.creator')}</span>
                  <strong>{selectedSchedule.creatorName}</strong>
                </div>
                <div className="schedule-detail-card">
                  <span>{t('schedule.columns.note')}</span>
                  <strong>{selectedSchedule.description || '-'}</strong>
                </div>
              </div>

              {selectedSchedule.scope === 'selected_users' && (
                <div className="schedule-detail-audience">
                  <h4>{t('schedule.detail.targetUsers')}</h4>
                  <div className="schedule-selected-target-list">
                    {resolveVisibleUserNames(selectedSchedule).map((name) => (
                      <span key={name} className="schedule-target-chip">{name}</span>
                    ))}
                    {!resolveVisibleUserNames(selectedSchedule).length && (
                      <p className="schedule-helper-text">{t('schedule.detail.noTargets')}</p>
                    )}
                  </div>
                </div>
              )}
              ...
              <form className="employee-create-form" onSubmit={createSchedule}>
                <div className="schedule-modal-layout">
                  <div className="form-grid-2 schedule-form-grid">
                    ...
                    <label className="recruitment-field">
                      <span>{t('schedule.form.endDate')}</span>
                      <input
                        type="date"
                        className="input-control"
                        value={form.endDate}
                  ...
                  <aside className="schedule-modal-preview">
                    ...
                    <p>{form.date || '-'} - {form.endDate || '-'} | {form.startTime} - {form.endTime}</p>
