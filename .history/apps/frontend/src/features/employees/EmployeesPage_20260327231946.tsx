...
type EmployeeView = 'profile' | 'personal' | 'departments' | 'teams' | 'organization' | 'contract' | 'performance' | 'education';
const VALID_EMPLOYEE_VIEWS: EmployeeView[] = ['profile', 'personal', 'departments', 'teams', 'organization', 'contract', 'performance', 'education'];
...
export function EmployeesPage() {
  ...
  const [selectedPersonal, setSelectedPersonal] = useState<EmployeeItem | null>(null);
  ...
  function renderPersonalView() {
    return (
      <>
        <p className="employees-meta">{t('employees.personal.description')}</p>
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
            ...
            <tbody>
              {pagedItems.map((item) => (
                <tr
                  ...
                  onClick={() => setSelectedPersonal(item)}
                >
                  <td>{item.id}</td>
                  <td>{item.fullName}</td>
                  <td>{item.email ?? '-'}</td>
  ...
  return (
    <section className="module-grid">
      <article className="card main-card">
        ...
        {selectedPersonal && (
          <div className="employee-modal-backdrop" onClick={() => setSelectedPersonal(null)}>
            <div className="delete-confirm-modal members-modal personal-modal" onClick={(e) => e.stopPropagation()}>
              <h3>{t('employees.personal.detailTitle')}</h3>
              
              {selectedPersonal.photoUrl && (
                <div className="personal-avatar-wrap">
                  <img src={selectedPersonal.photoUrl} alt="Avatar" className="personal-avatar" />
                </div>
              )}
              ...
              <div className="members-list">
                <div className="member-item">
                  ...
                  <span className="member-role">{selectedPersonal.fullName}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.columns.email')}</span>
                  <span className="member-role">{selectedPersonal.email ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.gender')}</span>
                  <span className="member-role">{selectedPersonal.gender ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.dateOfBirth')}</span>
                  <span className="member-role">
                    {selectedPersonal.dateOfBirth ? new Date(selectedPersonal.dateOfBirth).toLocaleDateString() : '-'}
                  </span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.nationalId')}</span>
                  <span className="member-role">{selectedPersonal.nationalId ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.address')}</span>
                  <span className="member-role">{selectedPersonal.address ?? '-'}</span>
                </div>
                ...
                <div className="member-item">
                  ...
                  <span className="member-role">{selectedPersonal.departmentId ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.columns.team')}</span>
                  <span className="member-role">{selectedPersonal.teamId ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.jobTitle')}</span>
                  <span className="member-role">{selectedPersonal.jobTitle ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.jobCategory')}</span>
                  <span className="member-role">{selectedPersonal.jobCategory ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.employeeStatus')}</span>
                  <span className="member-role">{selectedPersonal.employeeStatus ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.startDate')}</span>
                  <span className="member-role">
                    {selectedPersonal.startDate ? new Date(selectedPersonal.startDate).toLocaleDateString() : '-'}
                  </span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.columns.role')}</span>
                  <span className={`role-pill role-${selectedPersonal.role.replace('_', '-')}`}>
                    {selectedPersonal.role}
                  </span>
                </div>
              </div>
              <button className="mini-btn" onClick={() => setSelectedPersonal(null)}>
                {t('common.close') ?? 'Close'}
              </button>
            </div>
          </div>
          ...
          <div className="employee-modal-backdrop" onClick={() => setSelectedEducation(null)}>
            <div className="delete-confirm-modal members-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="members-list">
                ...
                <div className="member-item">
                  <span className="member-name">{t('employees.education.extraDegrees')}</span>
                  <span className="member-role">
                    {selectedEducation.degreeFiles?.length
                      ? t('employees.education.extraDegreesCount', {
                          count: selectedEducation.degreeFiles.length,
                        })
                      : t('employees.education.noExtraDegrees')}
                  </span>
                </div>
                {selectedEducation.degreeFiles?.length ? (
                  <div className="file-list-preview">
                    {selectedEducation.degreeFiles.map((file, index) => (
