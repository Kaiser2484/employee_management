...
import { getEmployees, EmployeeItem, createEmployee, CreateEmployeePayload, getAdminCatalogs } from '../../app/api';
        ...
        {!isLoading && !error && (
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {pagedItems.map((item) => (
                  <tr key={item.id} className="clickable-card" onClick={() => setSelectedEmployee(item)}>
                    <td>{item.id}</td>
                    <td>{item.fullName}</td>
                    <td>{item.email ?? '-'}</td>
                    <td>
  ...
  return (
    <section className="module-grid">
      <article className="card main-card">
        ...
        {selectedEmployee && (
          <div className="employee-modal-backdrop" onClick={() => setSelectedEmployee(null)}>
            <div className="delete-confirm-modal members-modal personal-modal" onClick={(e) => e.stopPropagation()}>
              <h3>{t('employees.personal.detailTitle', 'Employee Details')}</h3>
              
              {selectedEmployee.photoUrl && (
                <div className="personal-avatar-wrap">
                  <img src={selectedEmployee.photoUrl} alt="Avatar" className="personal-avatar" />
                </div>
              )}

              <div className="members-list">
                <div className="member-item">
                  <span className="member-name">{t('employees.columns.name')}</span>
                  <span className="member-role">{selectedEmployee.fullName}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.columns.email')}</span>
                  <span className="member-role">{selectedEmployee.email ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.gender', 'Gender')}</span>
                  <span className="member-role">{selectedEmployee.gender ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.dateOfBirth', 'Date of Birth')}</span>
                  <span className="member-role">
                    {selectedEmployee.dateOfBirth ? new Date(selectedEmployee.dateOfBirth).toLocaleDateString() : '-'}
                  </span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.nationalId', 'National ID')}</span>
                  <span className="member-role">{selectedEmployee.nationalId ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.address', 'Address')}</span>
                  <span className="member-role">{selectedEmployee.address ?? '-'}</span>
                </div>
                ...
                <div className="member-item">
                  ...
                  <span className="member-role">{selectedEmployee.departmentId ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.columns.team')}</span>
                  <span className="member-role">{selectedEmployee.teamId ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.jobTitle', 'Job Title')}</span>
                  <span className="member-role">{selectedEmployee.jobTitle ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.jobCategory', 'Job Category')}</span>
                  <span className="member-role">{selectedEmployee.jobCategory ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.employeeStatus', 'Employee Status')}</span>
                  <span className="member-role">{selectedEmployee.employeeStatus ?? '-'}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.personal.fields.startDate', 'Start Date')}</span>
                  <span className="member-role">
                    {selectedEmployee.startDate ? new Date(selectedEmployee.startDate).toLocaleDateString() : '-'}
                  </span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.education.columns.degree', 'Degree')}</span>
                  <span className="member-role">{selectedEmployee.degree || t('employees.education.noDegree')}</span>
                </div>
                <div className="member-item">
                  <span className="member-name">{t('employees.education.extraDegrees', 'Extra Degrees')}</span>
                  <span className="member-role">
                    {selectedEmployee.degreeFiles?.length
                      ? t('employees.education.extraDegreesCount', {
                          count: selectedEmployee.degreeFiles.length,
                        })
                      : t('employees.education.noExtraDegrees', 'None')}
                  </span>
                </div>
                {selectedEmployee.degreeFiles?.length ? (
                  <div className="file-list-preview">
                    {selectedEmployee.degreeFiles.map((file, index) => (
                      <a key={index} className="file-preview-item" href={file.url} download={file.name} target="_blank" rel="noreferrer">
                        {file.name}
                      </a>
                    ))}
                  </div>
                ...
                <div className="member-item">
                  <span className="member-name">{t('employees.columns.role')}</span>
                  <span className={`role-pill role-${selectedEmployee.role.replace('_', '-')}`}>
                    {selectedEmployee.role}
                  </span>
                </div>
              </div>
              <button className="mini-btn" onClick={() => setSelectedEmployee(null)}>
                {t('common.close') ?? 'Close'}
              </button>
            </div>
          </div>
