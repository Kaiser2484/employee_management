...
export function RecruitmentPage() {
  ...
  const [newCandidate, setNewCandidate] = useState({ fullName: '', email: '', phone: '', position: '', source: 'linkedin' as CandidateSource });
  ...
  const pipelineMap = useMemo(() => {
    return stageOrder.reduce<Record<CandidateStage, RecruitmentCandidate[]>>((acc, stage) => {
  ...
  function addCandidate(event: FormEvent) {
    event.preventDefault();
    if (newCandidate.fullName.trim().length < 3 || !newCandidate.email.trim()) {
    ...
    setNewCandidate({ fullName: '', email: '', phone: '', position: '', source: 'linkedin' });
          ...
          <div className="employee-modal-backdrop" onClick={() => setIsCandidateModalOpen(false)}>
            <section className="employee-modal" onClick={(event) => event.stopPropagation()}>
              <div className="employee-modal-head">
                ...
                <button type="button" className="ghost-btn" onClick={() => setIsCandidateModalOpen(false)}>{t('common.close')}</button>
              </div>
              <form className="employee-create-form" onSubmit={addCandidate}>
                <input className="input-control" placeholder={t('recruitment.form.candidateName')} value={newCandidate.fullName} onChange={(e) => setNewCandidate((prev) => ({ ...prev, fullName: e.target.value }))} />
                <input type="email" className="input-control" placeholder={t('recruitment.form.email')} value={newCandidate.email} onChange={(e) => setNewCandidate((prev) => ({ ...prev, email: e.target.value }))} />
                <input className="input-control" placeholder={t('recruitment.form.phone')} value={newCandidate.phone} onChange={(e) => setNewCandidate((prev) => ({ ...prev, phone: e.target.value }))} />
                <input className="input-control" placeholder={t('recruitment.form.positionTitle')} value={newCandidate.position} onChange={(e) => setNewCandidate((prev) => ({ ...prev, position: e.target.value }))} />
