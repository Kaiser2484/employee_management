...
import { BriefcaseBusiness, CalendarClock, CircleDashed, FileText, Plus, Sparkles, UploadCloud, Users } from 'lucide-react';
import { getRecruitmentCandidates, RecruitmentCandidate } from '../../app/api';
...
function calculateCandidateFitScore(input: {
  fullName: string;
  email: string;
  phone: string;
  position: string;
  ...
  let score = 40;
  if (input.fullName.trim().length >= 3) score += 10;
  if (/^\S+@\S+\.\S+$/.test(input.email.trim())) score += 12;
  if (input.phone.trim().replace(/[^0-9]/g, '').length >= 9) score += 8;
  if (input.position.trim().length >= 3) score += 10;
  if (input.avatarUrl) score += 8;
  if (input.cvFileName) score += 10;
  if (input.source === 'referral') score += 7;
  if (input.source === 'linkedin') score += 5;
  if (input.source === 'agency') score += 3;

  const positionText = input.position.toLowerCase();
  if (positionText.includes('senior') || positionText.includes('lead')) score += 5;
...
export function RecruitmentPage() {
  ...
  const candidateFitScore = useMemo(() => {
    return calculateCandidateFitScore(newCandidate);
  }, [newCandidate]);
  ...
  function onAvatarDrop(event: React.DragEvent<HTMLDivElement>) {
    ...
    if (file && file.type.startsWith('image/')) {
      handleCandidateAvatarChange(file);
    }
  }

  function onCvDrop(event: React.DragEvent<HTMLDivElement>) {
    event.preventDefault();
    setIsCvDropActive(false);
    const file = event.dataTransfer.files?.[0] ?? null;
    if (!file) return;
    const isAccepted = /\.(pdf|doc|docx)$/i.test(file.name);
    if (isAccepted) {
      handleCandidateCvChange(file);
    }
  }
                ...
                <tr key={candidate.id}>
                  ...
                  <td>
                    <div className="candidate-name-cell">
                      ...
                      />
                      <div className="candidate-name-meta">
                        <span>{candidate.fullName}</span>
                        <small className="fit-score-line">
                          <Sparkles size={12} /> {t('recruitment.fitScore')}: {candidate.fitScore ?? 70}/100
                        </small>
                      </div>
                    </div>
                  </td>
            ...
            <div className="recruitment-stage-list">
              {pipelineMap[stage].map((candidate) => (
                <article className="recruitment-candidate-card" key={candidate.id}>
                  ...
                  <p className="fit-score-line"><Sparkles size={12} /> {t('recruitment.fitScore')}: {candidate.fitScore ?? 70}/100</p>
                ...
                <div className="recruitment-modal-layout">
                  ...
                  <aside className="recruitment-preview-panel">
                    ...
                    <div className="fit-score-badge">
                      <Sparkles size={14} /> {t('recruitment.fitScore')}: {candidateFitScore}/100
                    </div>
                    ...
                    <div
                      className={`recruitment-dropzone ${isAvatarDropActive ? 'active' : ''}`}
                      onDragOver={(e) => { e.preventDefault(); setIsAvatarDropActive(true); }}
                      onDragLeave={() => setIsAvatarDropActive(false)}
                      onDrop={onAvatarDrop}
                    >
                      <UploadCloud size={16} />
                      <div>
                        <strong>{t('recruitment.form.avatar')}</strong>
                        <small>{t('recruitment.form.avatarDropHint')}</small>
                      </div>
                      <input type="file" accept="image/*" onChange={(e) => handleCandidateAvatarChange(e.target.files?.[0] ?? null)} />
                    </div>
                    <div
                      className={`recruitment-dropzone ${isCvDropActive ? 'active' : ''}`}
                      onDragOver={(e) => { e.preventDefault(); setIsCvDropActive(true); }}
                      onDragLeave={() => setIsCvDropActive(false)}
                      onDrop={onCvDrop}
                    >
                      <FileText size={16} />
                      <div>
                        <strong>{t('recruitment.form.cv')}</strong>
                        <small>{newCandidate.cvFileName || t('recruitment.form.cvDropHint')}</small>
                      </div>
