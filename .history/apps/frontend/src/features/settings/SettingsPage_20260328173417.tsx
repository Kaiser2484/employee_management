...
export function SettingsPage() {
  ...
  function handleAddEducation(event: FormEvent) {
    ...
    if (editingEducationId) {
      nextItems = educationHistory.map((item) =>
    ...
    setEditingEducationId(null);
    showToast(editingEducationId ? t('settings.educationUpdated', 'Cập nhật học vấn thành công.') : t('settings.educationAdded'), 'success');
  }

  function handleEditEducation(itemId: string) {
    const target = educationHistory.find((item) => item.id === itemId);
    if (!target) return;
    setEditingEducationId(itemId);
    setEducationDraft({
      level: target.level,
  ...
  function handleCancelEducationEdit() {
    setEditingEducationId(null);
    setEducationDraft(EMPTY_EDUCATION_DRAFT);
  }
  ...
  return (
    ...
    >
      ...
      <div className="settings-grid">
        ...
        <section className="card soft settings-card settings-education-card">
          <h3 className="settings-education-title">{t('settings.educationSection')}</h3>
          <p className="actor-text settings-education-description">
            {t('settings.educationDescription')}
          </p>

          <form onSubmit={handleAddEducation} className="settings-form settings-education-form">
            ...
            <div className="settings-actions settings-education-form-actions">
              <button type="submit" className="ghost-btn settings-education-submit-btn">
                {editingEducationId
                  ? t('settings.updateEducation', 'Cập nhật học vấn')
                  : t('settings.addEducation')}
              </button>
              {editingEducationId && (
                <button type="button" className="ghost-btn" onClick={handleCancelEducationEdit}>
                  {t('common.cancel', 'Hủy')}
                </button>
              )}
            </div>
          ...
          <div className="settings-education-list">
            {educationHistory.length === 0 ? (
              <p className="actor-text settings-education-empty" style={{ margin: 0 }}>
                {t('settings.educationEmpty')}
              </p>
            ) : (
              educationHistory.map((item) => (
                <article key={item.id} className="settings-education-item">
                  <div className="settings-education-meta">
                    <strong>{item.level}</strong>
                    <p className="actor-text" style={{ margin: '0.2rem 0 0' }}>
                      {item.school} - {item.major}
                    </p>
                    {(item.fromYear || item.toYear) && (
                      <p className="actor-text settings-education-years" style={{ margin: '0.2rem 0 0' }}>
                        {item.fromYear || '...'} - {item.toYear || '...'}
                      </p>
                    )}
                  </div>
                  <div className="settings-education-actions">
                    <button
                      type="button"
                      className="ghost-btn"
