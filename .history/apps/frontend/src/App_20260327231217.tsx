...
import { Users, CalendarDays, Briefcase, Globe, LogOut, Search, ChevronLeft, ChevronRight, ChevronDown, Settings, Building, ShieldCheck } from 'lucide-react';
...
function App() {
  ...
  const filteredNavItems = useMemo(() => {
    if (!searchQuery.trim()) return availableNavItems;
    const lowerQuery = searchQuery.toLowerCase();
    return availableNavItems.filter((item) => item.label.toLowerCase().includes(lowerQuery));
  }, [availableNavItems, searchQuery]);
  ...
  const subModuleItems = useMemo(() => {
    const map: Record<string, Array<{ label: string; path: string }>> = {
      employees: [
        { label: t('subMenu.employees.personal'), path: '/employees?view=personal' },
        ...
        { label: t('subMenu.employees.education'), path: '/employees?view=education' },
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-top">
          <div className="brand-wrap">
            ...
            {isSidebarExpanded && <span className="brand-text">HR Portal</span>}
          </div>
