...
export async function getCurrentUser(actor: Actor) {
  return request<{ user: Actor; message: string }>('/auth/me', actor);
}
...
export async function getProfileDetails(actor: Actor) {
  return request<{ data: ProfileDetails; message: string }>('/profile/me', actor);
}
...
export type AdminCatalogs = Record<CatalogView, string[]>;
...
export async function getTasks(actor: Actor, options?: { history?: boolean }) {
  const historyQuery = options?.history ? '?history=true' : '';
  return request<{ data: TaskItem[]; message: string }>(`/tasks${historyQuery}`, actor);
}
...
export async function confirmTaskCompletion(actor: Actor, taskId: string) {
  return request<{ data: TaskItem; message: string }>(`/tasks/${taskId}/confirm-completion`, actor, {
    method: 'PATCH',
  });
}

export async function getRecruitmentCandidates(actor: Actor) {
  return request<{ data: RecruitmentCandidate[]; message: string }>(`/recruitment/candidates`, actor);
}
...
export async function getAdminCatalogs(actor: Actor) {
  return request<{ data: AdminCatalogs; message: string }>(`/catalogs`, actor);
}

export async function addAdminCatalogItem(actor: Actor, type: CatalogView, value: string) {
  return request<{ data: { type: CatalogView; value: string }; message: string }>(`/catalogs/${type}`, actor, {
    method: 'POST',
    body: JSON.stringify({ value }),
  });
}

export async function removeAdminCatalogItem(actor: Actor, type: CatalogView, value: string) {
  return request<{ data: { type: CatalogView; value: string }; message: string }>(`/catalogs/${type}`, actor, {
    method: 'DELETE',
    body: JSON.stringify({ value }),
  });
