...
export class TasksService {
  ...
  private getStatusLifecyclePatch(currentStatus: TaskStatus, nextStatus: TaskStatus) {
    ...
    return patch as Prisma.TaskUpdateInput;
  }
  ...
  async findAll(actor: RequestUser, historyOnly = false) {
    ...
    const visibilityWhere = (historyOnly
      ? {
  ...
  async updateStatus(actor: RequestUser, taskId: string, status: TaskStatus) {
    ...
    if ((task as { completionConfirmedAt?: Date | null }).completionConfirmedAt) {
  ...
  async confirmCompletion(actor: RequestUser, taskId: string) {
    ...
    const updated = await this.prisma.task.update({
      ...
      data: {
        ...
        endedAt: (task as { endedAt?: Date | null }).endedAt ?? new Date(),
        startedAt: (task as { startedAt?: Date | null }).startedAt ?? task.createdAt,
      } as Prisma.TaskUpdateInput,
      include: this.taskInclude,
    });

    return {
      data: this.toResponseTask(updated as TaskWithRelations),
