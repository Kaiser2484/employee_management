...
export class TasksController {
  ...
  async findAll(@Req() req: Request & { user?: RequestUser }) {
    ...
    return this.tasksService.findAll(req.user!, history);
  }

  @Get('assignees')
  async searchAssignees(@Req() req: Request & { user?: RequestUser }) {
    const query = typeof req.query.query === 'string' ? req.query.query : '';
    return this.tasksService.searchAssignees(req.user!, query);
  }
  ...
  @Patch(':taskId/confirm-completion')
  async confirmCompletion(
    @Req() req: Request & { user?: RequestUser },
    @Param('taskId') taskId: string,
  ) {
    return this.tasksService.confirmCompletion(req.user!, taskId);
  }
}
