...
const validCatalogViews: CatalogView[] = ['job-title', 'departments', 'employee-status', 'job-categories'];

@Injectable()
export class CatalogsService {
  constructor(private readonly prisma: PrismaService) {}

  private parseCatalogView(type: string): CatalogView {
    if (validCatalogViews.includes(type as CatalogView)) {
      return type as CatalogView;
    }
  ...
  async addItem(type: string, value: string) {
    const catalogView = this.parseCatalogView(type);
    const normalizedValue = value.trim();
    if (!normalizedValue) {
      throw new BadRequestException('Catalog value is required');
    }

    const catalogType = catalogTypeMap[catalogView];
  ...
  async removeItem(type: string, value: string) {
    const catalogView = this.parseCatalogView(type);
    const normalizedValue = value.trim();
    if (!normalizedValue) {
      throw new BadRequestException('Catalog value is required');
