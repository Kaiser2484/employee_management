...
type CatalogView = 'job-title' | 'employee-status' | 'job-categories';
...
function loadCatalogsFromStorage(): Record<CatalogView, string[]> {
  const defaults = buildDefaultCatalogs();
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<Record<CatalogView, string[]>>;
    return {
...
export function AdministrationPage() {
  ...
  const [catalogs, setCatalogs] = useState<Record<CatalogView, string[]>>(loadCatalogsFromStorage);
  ...
  useEffect(() => {
    localStorage.setItem(ADMIN_CATALOGS_STORAGE_KEY, JSON.stringify(catalogs));
  }, [catalogs]);
  ...
  function addCatalogItem(view: CatalogView) {
    ...
    setCatalogs((current) => ({
      ...current,
      [view]: [normalized, ...current[view]],
    }));
    setCatalogDraft('');
    showToast(t('administration.catalog.added'), 'success');
  }

  function removeCatalogItem(view: CatalogView, target: string) {
    setCatalogs((current) => ({
      ...current,
